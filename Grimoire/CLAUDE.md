...
5. **No visual identity anywhere in the app** — worth calling out as its own root-cause finding
   ...
   #8). Contrast with the sibling `eso-build-manager` project's `build_sheet.py`, which uses
   ...
   `QFrame`s matching `eso-build-manager`) is still open if wanted later.
...
15. ✅ DONE — **Backup directory's default location isn't surfaced anywhere in the UI** —
    `backup_tab.py:106` silently falls back to `~/grimoire-backups` when `config.backup_dir` is
    empty, but `settings_dialog.py:57`'s placeholder text just says "Where to save backups…" with
...
16. Not actioned — **Update banner not visually verifiable this pass**, `main_window.py:50-69`'s
    `_update_banner` (bare "✕" dismiss button, `palette(highlight)`/`palette(highlightedText)`
    background) never appeared in any of the five screenshots since no update was pending during
    capture. Worth a dedicated screenshot in the next review to check contrast and the dismiss
    button's hit-target size.
