...
from eso_build_manager.constants import CLASS_COLORS, CP_TREE_COLORS, QUALITY_COLORS
from eso_build_manager import gold_history, dailies
...
TRAY_ICON_FILE = os.path.join(_DIR, 'packaging', 'eso-build-manager.svg')
...
# ~/.config/eso-build-manager/sync.json, e.g.:
...
_SYNC_CONFIG_FILE = os.path.expanduser('~/.config/eso-build-manager/sync.json')


def _load_sync_config() -> dict:
    try:
        with open(_SYNC_CONFIG_FILE, encoding='utf-8') as f:
            return json.load(f)
...
def _pull_save_file() -> None:
    ...
    host = cfg.get('host')
    if not remote_dir:
        return  # no ~/.config/eso-build-manager/sync.json yet -- nothing to pull
...
def _tab_dailies(chars: list[model.Character], on_change=None) -> QTableView:
    ...
    and the untracked-set persistence live in eso_build_manager.dailies,
...
class MainWindow(QMainWindow):
    def __init__(self, extra_tab: tuple[QWidget, str] | None = None):
        ...
        # (currently Grimoire's own MainWindow, titled "Addons") to sit alongside
        # Character/Activities/Storage/Collection below, instead of being a
        # separate outer tab one level up. Standalone runs of this app pass
        # nothing and get the same 4 tabs as always. Kept alive across
        ...
        # Serenity', 'ESOBuildManager') used to be used here, which silently wrote to
        # a DIFFERENT file (~/.config/CubicSerenity/ESOBuildManager.conf, no space)
        # than app.py's setApplicationName("ESO Build Manager") default (WITH a space)
        # -- real confusion during tonight's debugging, worth never repeating.
    ...
    def _setup_tray(self):
        ...
        self._tray.setToolTip('ESO Build Manager')
    ...
    def _init_save_watcher(self):
        ...
        the same ~/.config/eso-build-manager/sync.json remote_dir
    ...
    def _load_local_data(self) -> dict:
        ...
        per-resource sync-server download methods (server/, eso_build_manager/
...
# would mean two writers hitting the same SQLite build DB and two independent
# addon-push timers racing each other against the sync server. A QLocalServer/
# QLocalSocket handshake is the standard Qt way to detect and defer to an
# already-running instance instead.
_SINGLE_INSTANCE_KEY = 'eso-build-manager-single-instance'
