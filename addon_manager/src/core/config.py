"""Persistent app configuration stored as JSON."""

from __future__ import annotations

import json
import os
import sys
from pathlib import Path
from typing import Any


def _base_dir() -> Path:
    if sys.platform == "win32":
        return Path(os.environ.get("APPDATA", Path.home()))
    return Path.home() / ".config"


def _config_dir() -> Path:
    return _base_dir() / "eso-helper-addons"


def _config_path() -> Path:
    return _config_dir() / "config.json"


# Pre-rename config dir (this was "Grimoire" before it became a vendored,
# de-branded part of ESO Helper -- see eso-helper CLAUDE.md). Read once as a
# migration source if the new path has never been written; never written to.
_LEGACY_CONFIG_PATH = _base_dir() / "grimoire" / "config.json"


_DEFAULTS: dict[str, Any] = {
    "addons_dir": "",
    "backup_dir": "",
    "saved_vars_dir": "",
    "backup_include_saved_vars": False,
    "theme": "dark",
    "browse_sort_column": 4,   # COL_DOWNLOADS
    "browse_sort_order": "desc",
    "tray_enabled": True,
}


class Config:
    def __init__(self) -> None:
        self._data: dict[str, Any] = dict(_DEFAULTS)
        self._load()

    def _load(self) -> None:
        path = _config_path()
        if path.exists():
            try:
                self._data.update(json.loads(path.read_text()))
                return
            except (json.JSONDecodeError, OSError):
                pass
        # First run under the new name -- one-time migration from the old
        # "grimoire" config dir, if it exists. Never touches/deletes the old file.
        if _LEGACY_CONFIG_PATH.exists():
            try:
                self._data.update(json.loads(_LEGACY_CONFIG_PATH.read_text()))
                self._data.pop("auto_update_on_launch", None)  # feature removed
            except (json.JSONDecodeError, OSError):
                pass

    def save(self) -> None:
        path = _config_path()
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(self._data, indent=2))

    def get(self, key: str, default: Any = None) -> Any:
        return self._data.get(key, default)

    def set(self, key: str, value: Any) -> None:
        self._data[key] = value

    @property
    def addons_dir(self) -> str:
        return self._data.get("addons_dir", "")

    @addons_dir.setter
    def addons_dir(self, v: str) -> None:
        self._data["addons_dir"] = v

    @property
    def backup_dir(self) -> str:
        return self._data.get("backup_dir", "")

    @backup_dir.setter
    def backup_dir(self, v: str) -> None:
        self._data["backup_dir"] = v

    @property
    def saved_vars_dir(self) -> str:
        return self._data.get("saved_vars_dir", "")

    @saved_vars_dir.setter
    def saved_vars_dir(self, v: str) -> None:
        self._data["saved_vars_dir"] = v

    @property
    def backup_include_saved_vars(self) -> bool:
        return bool(self._data.get("backup_include_saved_vars", False))

    @backup_include_saved_vars.setter
    def backup_include_saved_vars(self, v: bool) -> None:
        self._data["backup_include_saved_vars"] = v

    @property
    def browse_sort_column(self) -> int:
        try:
            return int(self._data.get("browse_sort_column", 4))
        except (ValueError, TypeError):
            return 4

    @browse_sort_column.setter
    def browse_sort_column(self, v: int) -> None:
        self._data["browse_sort_column"] = v

    @property
    def browse_sort_order(self) -> str:
        return self._data.get("browse_sort_order", "desc")

    @browse_sort_order.setter
    def browse_sort_order(self, v: str) -> None:
        self._data["browse_sort_order"] = v

    @property
    def tray_enabled(self) -> bool:
        return bool(self._data.get("tray_enabled", True))

    @tray_enabled.setter
    def tray_enabled(self, v: bool) -> None:
        self._data["tray_enabled"] = v


