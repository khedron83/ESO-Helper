...
def create_backup(
    ...
    zip_path = backup_dir / f"eso-helper-addons_{timestamp}{suffix}.zip"
...
def list_backups(backup_dir: Path) -> list[Path]:
    """Return backup zips sorted newest-first. Matches both the current
    filename prefix and the pre-rename "grimoire_" one, so backups made before
    the app was renamed off Grimoire still show up and remain restorable."""
    if not backup_dir.exists():
        return []
    return sorted(
        {*backup_dir.glob("eso-helper-addons_*.zip"), *backup_dir.glob("grimoire_*.zip")},
