...
from PySide6.QtWidgets import (
    ...
    QLabel, QProgressBar, QTextBrowser, QSplitter, QComboBox, QStackedLayout,
)
...
from .style import TABLE_STYLE, UPDATE_COLOR, INSTALLED_COLOR, PRIMARY_BUTTON_STYLE
...
class BrowseTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        dl = QVBoxLayout(self._detail_content)
        ...
        self._detail_meta.setStyleSheet("color: palette(windowText); font-size: 11px;")
        ...
        self._detail_empty = QLabel("Select an addon to see its details.")
        self._detail_empty.setAlignment(Qt.AlignCenter)
        self._detail_empty.setStyleSheet("color: palette(placeholderText); font-size: 12px;")
        ...
        detail = QWidget()
        detail.setLayout(self._detail_stack)
    ...
    def _apply_install_color(self, item: QTableWidgetItem, info: RemoteAddonInfo):
        ...
        if inst:
            if inst.version and info.version and inst.version != info.version:
                ...
                item.setForeground(INSTALLED_COLOR)
    ...
    def _update_install_button(self, info: RemoteAddonInfo):
        ...
        if inst and inst.version and info.version and inst.version != info.version:
            ...
            self._btn_install.setStyleSheet(PRIMARY_BUTTON_STYLE)
        elif inst:
