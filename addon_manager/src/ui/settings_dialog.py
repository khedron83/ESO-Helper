...
class SettingsDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self._addons_dir.textChanged.connect(self._validate_addons_dir)
        ...
        self._addons_dir_warning = QLabel("This path doesn't exist.")
        self._addons_dir_warning.setStyleSheet(f"color: {ERROR_COLOR}; font-size: 11px;")
        self._addons_dir_warning.hide()
        form.addRow("", self._addons_dir_warning)
        ...
        self._backup_saved_vars = QCheckBox("Include SavedVariables in backups by default")
        ...
        self._backup_dir.setPlaceholderText(
            f"Default: {Path.home() / 'eso-helper-addon-backups'}"
        )
        ...
        # Options
        self._tray_enabled = QCheckBox("Show tray icon (minimize instead of closing)")
        self._tray_enabled.setChecked(self.config.tray_enabled)
        form.addRow("", self._tray_enabled)
        ...
        self._validate_addons_dir()

    def _validate_addons_dir(self):
        text = self._addons_dir.text().strip()
        invalid = bool(text) and not Path(text).is_dir()
        self._addons_dir.setStyleSheet(f"border: 1px solid {ERROR_COLOR};" if invalid else "")
        self._addons_dir_warning.setVisible(invalid)
    ...
    def _save(self):
        ...
        self.config.tray_enabled = self._tray_enabled.isChecked()
