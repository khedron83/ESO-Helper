...
class BackupTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._chk_sv = QCheckBox("Include SavedVariables for this backup")
        ...
        self._empty_label = QLabel("No backups yet — click “Create Backup” to make your first one.")
    ...
    def refresh(self):
        ...
        # Only seed from the config default once — after that the checkbox is
        # a per-session choice the user controls directly, not something that
        # should silently reset every time this tab becomes active again.
        if not self._sv_initialized:
            self._chk_sv.setChecked(self.config.backup_include_saved_vars)
            self._sv_initialized = True
        ...
        for path in backups:
            ...
            self._table.setItem(row, 1, _SizeItem(f"{size_mb:.1f} MB", size_bytes))
            mtime_str = datetime.datetime.fromtimestamp(st.st_mtime).strftime("%Y-%m-%d %H:%M")
            self._table.setItem(row, 2, _DateItem(mtime_str, st.st_mtime))
            self._table.item(row, 0).setData(Qt.UserRole, str(path))
        self._table.setSortingEnabled(True)
        self._table_stack.setCurrentWidget(self._empty_label if not backups else self._table)
    ...
    def _backup_dir(self) -> Path | None:
        ...
        # No configured backup_dir -- use the default, but keep using the
        # pre-rename default (~/grimoire-backups) if it's the one that
        # actually already has backups in it, so existing backups aren't
        # orphaned by the app's rename off "Grimoire".
        legacy_default = Path.home() / "grimoire-backups"
        if legacy_default.is_dir() and any(legacy_default.glob("grimoire_*.zip")):
            return legacy_default
        return Path.home() / "eso-helper-addon-backups"
